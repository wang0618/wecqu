...
Page({
    ...
    sendRequest: function () {
        ...
        //判断并读取缓存
        if (app.cache.card) {
            yktRender(app.cache.card);
        }
        if (app.cache.card_balance) {
            _this.setData({
                balance: app.cache.card_balance.balance,
                last_time: app.cache.card_balance.time
            });
        }
        ...
        wx.request({
            url: app._server + app.api.get_cardcost,
            method: 'POST',
            data: app.key({}),
            success: function (res) {
            ...
            fail: function (res) {
                if (!app.cache.card) {
                    ...
                    _this.toast(res.data.message || '更新数据失败');
                }
            },
            complete: function () {
